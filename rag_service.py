...
import json
import re                      # ✅ 추가
from rank_bm25 import BM25Okapi  # ✅ 추가
...
def _load_paju_docs() -> List[Dict[str, str]]:
    """
    DATA_PATHS(dict) 안의 모든 json/jsonl 파일을 읽어서 문서 리스트로 변환.
...
def _normalize_doc(obj: Dict[str, str], source: str) -> Dict[str, str] | None:
    ...
    if source == "carecenter":
        ...
        # content: 검색용 텍스트 블록 (RAG가 물어볼 내용)
        content_lines = [
...
# BM25용 텍스트: title + content 같이 사용해도 됨
BM25_TEXTS: List[str] = [
...
def _keyword_score(query: str, text: str) -> int:
    ...
    공백 기준으로 나눈 단어들이 text 안에 얼마나 포함되는지 카운트.
    """
...
def retrieve_context(query: str) -> str:
    """
    BM25로 질문과 가장 유사한 문서 1개를 찾아서
    그 문서의 content만 반환 (LLM 컨텍스트용)
    """
    if not PAJU_DOCS:
        return ""
